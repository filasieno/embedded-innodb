...
Parser_system g_parser_system{};
Parser_system *g_parser_system_current = &g_parser_system;

#ifdef UNIV_SQL_DEBUG
bool &pars_print_lexed = g_parser_system.pars_print_lexed;
#endif /* UNIV_SQL_DEBUG */

void pars_system_init(Parser_system *sys) {
#ifdef UNIV_SQL_DEBUG
  sys->pars_print_lexed = false;
#endif /* UNIV_SQL_DEBUG */
  sys->pars_to_char_token.code = PARS_TO_CHAR_TOKEN;
  sys->pars_to_number_token.code = PARS_TO_NUMBER_TOKEN;
  sys->pars_to_binary_token.code = PARS_TO_BINARY_TOKEN;
  sys->pars_binary_to_number_token.code = PARS_BINARY_TO_NUMBER_TOKEN;
  sys->pars_substr_token.code = PARS_SUBSTR_TOKEN;
  sys->pars_replstr_token.code = PARS_REPLSTR_TOKEN;
  sys->pars_concat_token.code = PARS_CONCAT_TOKEN;
  sys->pars_instr_token.code = PARS_INSTR_TOKEN;
  sys->pars_length_token.code = PARS_LENGTH_TOKEN;
  sys->pars_sysdate_token.code = PARS_SYSDATE_TOKEN;
  sys->pars_printf_token.code = PARS_PRINTF_TOKEN;
  sys->pars_assert_token.code = PARS_ASSERT_TOKEN;
  sys->pars_rnd_token.code = PARS_RND_TOKEN;
  sys->pars_rnd_str_token.code = PARS_RND_STR_TOKEN;
  sys->pars_count_token.code = PARS_COUNT_TOKEN;
  sys->pars_sum_token.code = PARS_SUM_TOKEN;
  sys->pars_distinct_token.code = PARS_DISTINCT_TOKEN;
  sys->pars_binary_token.code = PARS_BINARY_TOKEN;
  sys->pars_blob_token.code = PARS_BLOB_TOKEN;
  sys->pars_int_token.code = PARS_INT_TOKEN;
  sys->pars_char_token.code = PARS_CHAR_TOKEN;
  sys->pars_float_token.code = PARS_FLOAT_TOKEN;
  sys->pars_update_token.code = PARS_UPDATE_TOKEN;
  sys->pars_asc_token.code = PARS_ASC_TOKEN;
  sys->pars_desc_token.code = PARS_DESC_TOKEN;
  sys->pars_open_token.code = PARS_OPEN_TOKEN;
  sys->pars_close_token.code = PARS_CLOSE_TOKEN;
  sys->pars_share_token.code = PARS_SHARE_TOKEN;
  sys->pars_unique_token.code = PARS_UNIQUE_TOKEN;
  sys->pars_clustered_token.code = PARS_CLUSTERED_TOKEN;
  sys->pars_star_denoter = 12345678;
}
...
void pars_var_init(Parser_context* ctx) {
#ifdef UNIV_SQL_DEBUG
  g_parser_system.pars_print_lexed = false;
#endif /* UNIV_SQL_DEBUG */
  // pars_lexer_var_init(ctx);
  /* no global context in reentrant mode */
  /* These should really be const, so we simply check that they
  are set to the correct value. */
  ut_a(g_parser_system.pars_to_char_token.code == PARS_TO_CHAR_TOKEN);
  ut_a(g_parser_system.pars_to_number_token.code == PARS_TO_NUMBER_TOKEN);
  ut_a(g_parser_system.pars_to_binary_token.code == PARS_TO_BINARY_TOKEN);
  ut_a(g_parser_system.pars_binary_to_number_token.code == PARS_BINARY_TO_NUMBER_TOKEN);
  ut_a(g_parser_system.pars_substr_token.code == PARS_SUBSTR_TOKEN);
  ut_a(g_parser_system.pars_replstr_token.code == PARS_REPLSTR_TOKEN);
  ut_a(g_parser_system.pars_concat_token.code == PARS_CONCAT_TOKEN);
  ut_a(g_parser_system.pars_instr_token.code == PARS_INSTR_TOKEN);
  ut_a(g_parser_system.pars_length_token.code == PARS_LENGTH_TOKEN);
  ut_a(g_parser_system.pars_sysdate_token.code == PARS_SYSDATE_TOKEN);
  ut_a(g_parser_system.pars_printf_token.code == PARS_PRINTF_TOKEN);
  ut_a(g_parser_system.pars_assert_token.code == PARS_ASSERT_TOKEN);
  ut_a(g_parser_system.pars_rnd_token.code == PARS_RND_TOKEN);
  ut_a(g_parser_system.pars_rnd_str_token.code == PARS_RND_STR_TOKEN);
  ut_a(g_parser_system.pars_count_token.code == PARS_COUNT_TOKEN);
  ut_a(g_parser_system.pars_sum_token.code == PARS_SUM_TOKEN);
  ut_a(g_parser_system.pars_distinct_token.code == PARS_DISTINCT_TOKEN);
  ut_a(g_parser_system.pars_binary_token.code == PARS_BINARY_TOKEN);
  ut_a(g_parser_system.pars_blob_token.code == PARS_BLOB_TOKEN);
  ut_a(g_parser_system.pars_int_token.code == PARS_INT_TOKEN);
  ut_a(g_parser_system.pars_char_token.code == PARS_CHAR_TOKEN);
  ut_a(g_parser_system.pars_float_token.code == PARS_FLOAT_TOKEN);
  ut_a(g_parser_system.pars_update_token.code == PARS_UPDATE_TOKEN);
  ut_a(g_parser_system.pars_asc_token.code == PARS_ASC_TOKEN);
  ut_a(g_parser_system.pars_desc_token.code == PARS_DESC_TOKEN);
  ut_a(g_parser_system.pars_open_token.code == PARS_OPEN_TOKEN);
  ut_a(g_parser_system.pars_close_token.code == PARS_CLOSE_TOKEN);
  ut_a(g_parser_system.pars_share_token.code == PARS_SHARE_TOKEN);
  ut_a(g_parser_system.pars_unique_token.code == PARS_UNIQUE_TOKEN);
  ut_a(g_parser_system.pars_clustered_token.code == PARS_CLUSTERED_TOKEN);
 ...
 */
static func_node_t *pars_func_low(Parser_context* ctx, int func, que_node_t *arg) 
{
  auto node = reinterpret_cast<func_node_t *>( mem_heap_alloc(ctx->sym_tab->heap, sizeof(func_node_t)));
  ...
  node->args = arg;
  ctx->sym_tab->func_node_list.push_back(node);
  return node;
}

func_node_t *pars_func(Parser_context* ctx, que_node_t *res_word, que_node_t *arg)
{
  return pars_func_low(ctx, (reinterpret_cast<pars_res_word_t *>(res_word))->code, arg);
}

func_node_t *pars_op(Parser_context* ctx, int func, que_node_t *arg1, que_node_t *arg2) 
{
  que_node_list_add_last(nullptr, arg1);
  if (arg2 != nullptr) {
    que_node_list_add_last(arg1, arg2);
  }
  return pars_func_low(ctx, func, arg1);
}

order_node_t *pars_order_by(Parser_context* ctx, sym_node_t *column, pars_res_word_t *asc) 
{
  auto node = reinterpret_cast<order_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(order_node_t)));
  node->common.type = QUE_NODE_ORDER;
  node->column = column;
  if (asc == &g_parser_system.pars_asc_token) {
    node->asc = true;
  } else {
    ut_a(asc == &g_parser_system.pars_desc_token);
    node->asc = false;
  }
  return node;
}
 ...
 */
static void pars_resolve_exp_variables_and_types(Parser_context* ctx, sel_node_t *sel_node, que_node_t *exp_node)
{
  ...
  if (que_node_get_type(exp_node) == QUE_NODE_FUNC) {
    ...
    while (arg != nullptr) {
      pars_resolve_exp_variables_and_types(ctx, sel_node, arg);
  ...
  auto it = std::find_if(
      ctx->sym_tab->sym_list.begin(),
      ctx->sym_tab->sym_list.end(), [sym_node](const auto &node) {
  ...
  if (it == ctx->sym_tab->sym_list.end()) {
 ...
 */
static void pars_resolve_exp_list_variables_and_types(Parser_context* ctx, sel_node_t *sel_node, que_node_t *exp_node) noexcept
{
  while (exp_node != nullptr) {
    pars_resolve_exp_variables_and_types(ctx, sel_node, exp_node);
 ...
 */
static void pars_select_all_columns(Parser_context* ctx, sel_node_t *sel_node) noexcept {
  ...
  while (table_node != nullptr) {
    ...
    for (ulint i{}; i < table->get_n_user_cols(); ++i) {
      const auto col_name = table->get_col_name(i);
      auto col_node = sym_tab_add_id(ctx->sym_tab, (byte *)col_name,
                                     strlen(col_name));
...
sel_node_t *pars_select_list(Parser_context* ctx, que_node_t *select_list, sym_node_t *into_list) {
  auto sel_node = sel_node_t::create(ctx->sym_tab->heap);
  sel_node->m_select_list = select_list;
  sel_node->m_into_list = into_list;
  pars_resolve_exp_list_variables_and_types(ctx, nullptr, into_list);
  return sel_node;
}
...
sel_node_t *pars_select_statement(
  ...
  if (sel_node->m_select_list == &ctx->system->pars_star_denoter) {
    /* SELECT * FROM ... */
    pars_select_all_columns(ctx, sel_node);
  }
  ...
  pars_resolve_exp_list_variables_and_types(ctx, sel_node, sel_node->m_select_list);
  ...
  if (search_cond != nullptr) {
    ...
    pars_resolve_exp_variables_and_types(ctx, sel_node, search_cond);
  }
  ...
  opt_search_plan(ctx->sym_tab, sel_node);
  return sel_node;
}

que_node_t *pars_cursor_declaration(Parser_context* ctx, sym_node_t *sym_node, sel_node_t *sel_node) {
  (void)ctx;
  sym_node->resolved = true;
...
que_node_t *pars_function_declaration(Parser_context* ctx, sym_node_t *sym_node) {
  sym_node->resolved = true;
  sym_node->token_type = SYM_FUNCTION;
  /* Check that the function exists. */
  ut_a(pars_info_get_user_func(ctx->sym_tab->info, sym_node->name));
  return sym_node;
}

upd_node_t *pars_update_statement_start(Parser_context* ctx, bool is_delete, sym_node_t *table_sym, col_assign_node_t *col_assign_list) {
  auto node = srv_row_upd->node_create(ctx->sym_tab->heap);
...
col_assign_node_t *pars_column_assignment(Parser_context* ctx, sym_node_t *column, que_node_t *exp) {
  auto node = reinterpret_cast<col_assign_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(col_assign_node_t)));
 ...
 */
static void pars_process_assign_list(Parser_context* ctx, upd_node_t *node) noexcept {
  ...
  while (assign_node != nullptr) {
    ...
    pars_resolve_exp_variables_and_types(ctx, nullptr, assign_node->val);
#if 0
  ...
  node->m_update = Row_update::upd_create(n_assigns, ctx->sym_tab->heap);
...
upd_node_t *pars_update_statement(Parser_context* ctx, upd_node_t *node, sym_node_t *cursor_sym, que_node_t *search_cond) {
  auto table_sym = node->m_table_sym;
  ...
  if (cursor_sym != nullptr) {
    pars_resolve_exp_variables_and_types(ctx, nullptr, cursor_sym);
    sel_node = cursor_sym->alias->cursor_def;
    node->m_searched_update = false;
  } else {
    sel_node = pars_select_list(ctx, nullptr, nullptr);
    pars_select_statement(ctx, sel_node, table_sym, search_cond, nullptr, &g_parser_system.pars_share_token, nullptr);
  ...
  if (node->m_is_delete) {
    ...
    pars_process_assign_list(ctx, node);
  }
...
ins_node_t *pars_insert_statement(Parser_context* ctx, sym_node_t *table_sym, que_node_t *values_list, sel_node_t *sel_node) {
  ut_a(values_list != nullptr || sel_node != nullptr);
  ut_a(values_list == nullptr || sel_node == nullptr);
  auto ins_type = values_list != nullptr ? INS_VALUES : INS_SEARCHED;
  pars_retrieve_table_def(table_sym);
  auto ins_node = reinterpret_cast<ins_node_t *>(srv_row_ins->node_create(ins_type, table_sym->table, ctx->sym_tab->heap));
  auto row = dtuple_create(ctx->sym_tab->heap, ins_node->m_table->get_n_cols());
  ...
  if (values_list != nullptr) {
    pars_resolve_exp_list_variables_and_types(ctx, nullptr, values_list);
...
sym_node_t *pars_variable_declaration(Parser_context* ctx, sym_node_t *node, pars_res_word_t *type) {
  (void)ctx;
  node->resolved = true;
...
sym_node_t *pars_parameter_declaration(Parser_context* ctx, sym_node_t *node, ulint param_type,
                                       pars_res_word_t *type) {
  ut_a((param_type == PARS_INPUT) || (param_type == PARS_OUTPUT));
  pars_variable_declaration(ctx, node, type);
...
elsif_node_t *pars_elsif_element(Parser_context* ctx, que_node_t *cond, que_node_t *stat_list) {
  auto node = reinterpret_cast<elsif_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(elsif_node_t)));
  node->common.type = QUE_NODE_ELSIF;
  node->cond = cond;
  pars_resolve_exp_variables_and_types(ctx, nullptr, cond);
  node->stat_list = stat_list;
  return node;
}

if_node_t *pars_if_statement(Parser_context* ctx, que_node_t *cond, que_node_t *stat_list, que_node_t *else_part) {
  elsif_node_t *elsif_node;
  auto node = reinterpret_cast<if_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(if_node_t)));
...
while_node_t *pars_while_statement(Parser_context* ctx, que_node_t *cond, que_node_t *stat_list) {
  auto node = reinterpret_cast<while_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(while_node_t)));
...
for_node_t *pars_for_statement(Parser_context* ctx, sym_node_t *loop_var,
                               que_node_t *loop_start_limit,
                               que_node_t *loop_end_limit,
                               que_node_t *stat_list) {
  auto node = reinterpret_cast<for_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(for_node_t)));
  node->common.type = QUE_NODE_FOR;
  pars_resolve_exp_variables_and_types(ctx, nullptr, loop_var);
  pars_resolve_exp_variables_and_types(ctx, nullptr, loop_start_limit);
  pars_resolve_exp_variables_and_types(ctx, nullptr, loop_end_limit);
...
exit_node_t *pars_exit_statement(Parser_context* ctx) {
  auto node = reinterpret_cast<exit_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(exit_node_t)));
  node->common.type = QUE_NODE_EXIT;
  return node;
}

return_node_t *pars_return_statement(Parser_context* ctx) {
  auto node = reinterpret_cast<return_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(return_node_t)));
  node->common.type = QUE_NODE_RETURN;
  return node;
}

assign_node_t *pars_assignment_statement(Parser_context* ctx, sym_node_t *var, que_node_t *val) {
  auto node = reinterpret_cast<assign_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(assign_node_t)));
  node->common.type = QUE_NODE_ASSIGNMENT;
  node->var = var;
  node->val = val;
  pars_resolve_exp_variables_and_types(ctx, nullptr, var);
  pars_resolve_exp_variables_and_types(ctx, nullptr, val);
  ut_a(dtype_get_mtype(dfield_get_type(que_node_get_val(var))) ==
       dtype_get_mtype(dfield_get_type(que_node_get_val(val))));
  return node;
}

func_node_t *pars_procedure_call(Parser_context* ctx, que_node_t *res_word, que_node_t *args) {
  auto node = pars_func(ctx, res_word, args);
  pars_resolve_exp_list_variables_and_types(ctx, nullptr, args);
  return node;
}

fetch_node_t *pars_fetch_statement(Parser_context* ctx, sym_node_t *cursor, sym_node_t *into_list, sym_node_t *user_func) {
  /* Logical XOR. */
  ut_a(!into_list != !user_func);
  auto ptr = mem_heap_alloc(ctx->sym_tab->heap, sizeof(fetch_node_t));
  auto fetch_node = new (ptr) fetch_node_t();
  fetch_node->m_common.type = QUE_NODE_FETCH;
  pars_resolve_exp_variables_and_types(ctx, nullptr, cursor);
  if (into_list != nullptr) {
    pars_resolve_exp_list_variables_and_types(ctx, nullptr, into_list);
    fetch_node->m_into_list = into_list;
    fetch_node->m_func = nullptr;
  } else {
    pars_resolve_exp_variables_and_types(ctx, nullptr, user_func);
    fetch_node->m_func = pars_info_get_user_func(ctx->sym_tab->info, user_func->name);
...
open_node_t *pars_open_statement(Parser_context* ctx, ulint type, sym_node_t *cursor) {
  auto open_node = reinterpret_cast<open_node_t *>(
      mem_heap_alloc(ctx->sym_tab->heap, sizeof(open_node_t)));
...
row_printf_node_t *pars_row_printf_statement(Parser_context* ctx, sel_node_t *sel_node) {
  auto row_printf_node = reinterpret_cast<row_printf_node_t *>(mem_heap_alloc(ctx->sym_tab->heap, sizeof(row_printf_node_t)));
...
Commit_node *pars_commit_statement(Parser_context* ctx) {
  return Trx::commit_node_create(ctx->sym_tab->heap);
}

Trx_rollback *pars_rollback_statement(Parser_context* ctx) {
  return roll_node_create(ctx->sym_tab->heap);
}

sym_node_t *pars_column_def(Parser_context* ctx, sym_node_t *sym_node, pars_res_word_t *type, sym_node_t *len, void *is_unsigned, void *is_not_null) {
...
Table_node *pars_create_table(Parser_context* ctx, sym_node_t *table_sym, sym_node_t *column_defs,
  ...
  auto node =
      Table_node::create(srv_dict_sys, table, ctx->sym_tab->heap, true);
...
Index_node *pars_create_index(Parser_context* ctx, pars_res_word_t *unique_def,
  ...
  auto node =
      Index_node::create(srv_dict_sys, index, ctx->sym_tab->heap, true);
  table_sym->resolved = true;
  table_sym->token_type = SYM_TABLE;
...
que_fork_t *pars_procedure_definition(Parser_context* ctx, sym_node_t *sym_node,
                                      sym_node_t *param_list,
                                      que_node_t *stat_list) {
  auto heap = ctx->sym_tab->heap;
  ...
  node->stat_list = stat_list;
  pars_set_parent_in_list(stat_list, node);
  node->sym_tab = ctx->sym_tab;
  thr->child = node;
  ctx->sym_tab->query_graph = fork;
  return fork;
}

que_fork_t *pars_stored_procedure_call(Parser_context* /*ctx*/, sym_node_t *) {
  ut_error;
  return nullptr;
}

int pars_get_lex_chars(Parser_context* ctx, char *buf, int max_size) {
  auto len =
      ctx->sym_tab->string_len - ctx->sym_tab->next_char_pos;
  if (len == 0) {
    return 0;
  }
  ...
  memcpy(buf,
         ctx->sym_tab->sql_string + ctx->sym_tab->next_char_pos,
         len);
  ctx->sym_tab->next_char_pos += len;
  return static_cast<int>(len);
}

void yyerror(Parser_context* ctx, yyscan_t /*scanner*/, const char *p) {
  (void) ctx;
...
que_t *pars_sql(pars_info_t *info, const char *str) {
  ...
  Parser_context ctx{};
  ctx.sym_tab = sym_tab_create(heap);
  ctx.string_len = strlen(str);
  ctx.sql_string = static_cast<char *>(mem_heap_dup(heap, str, ctx.string_len + 1));
  ctx.next_char_pos = 0;
  ctx.info = info;
  yyscan_t scanner{};
  yylex_init_extra(&ctx, &scanner);
  auto buf = yy_scan_bytes(ctx.sql_string, static_cast<int>(ctx.string_len), scanner);
  yyparse(&ctx, scanner);
  yy_delete_buffer(buf, scanner);
  yylex_destroy(scanner);
  for (auto sym_node : ctx.sym_tab->sym_list) {
    ut_a(sym_node->resolved);
  }
  auto graph = ctx.sym_tab->query_graph;
  graph->info = info;
  graph->sym_tab = ctx.sym_tab;
  return graph;
}
