...
bool Dict::table_rename_in_cache(Table *table, const char *new_name, bool rename_also_foreigns) noexcept {
  ...
  for (auto foreign : table->m_foreign_list) {
    ...
    if (strchr(foreign->m_id, '/')) {
      ...
      const auto old_len = strlen(old_name);
      if (strlen(foreign->m_id) > old_len + (sizeof(dict_ibfk) - 1) &&
          memcmp(foreign->m_id, old_name, old_len) == 0 &&
          !memcmp(foreign->m_id + old_len, dict_ibfk, (sizeof dict_ibfk) - 1)) {
